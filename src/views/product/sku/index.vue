...
import {SkuData, SkuInfoData, SkuResponseData} from "@/api/product/sku/type.ts";
...
const removeSku=async (id:number)=>{
  let result:any=await reqRemoveSku()
  if(result.code==200){
...
<template>
  <el-card>
    <el-table border style="margin: 10px 0;" :data="skuArr">
      ...
      <el-table-column label="价格" width="160px" prop="price"></el-table-column>
      <el-table-column label="操作" width="260px" fixed="right">
        <template #="{row,$index}">
          <el-button :type="row.isSale==1?'success':'info'" size="small" :icon="row.isSale==1?'Bottom':'Top'" @click="updateSale(row)"></el-button>
    ...
    <el-drawer v-model="drawer">
      ...
      <template #default>
        ...
        <el-row style="margin: 10px 0;">
          ...
          <el-col :span="18">
            <el-tag style="margin: 0 5px;" v-for="(item,index) of skuInfo.skuAttrValueList" :key="item.id">{{item.valueName}}</el-tag>
          </el-col>
        </el-row>
        <el-row style="margin: 10px 0;">
          <el-col :span="6">销售属性</el-col>
          <el-col :span="18">
            <el-tag style="margin: 0 5px;" v-for="(item,index) of skuInfo.skuSaleAttrValueList" :key="item.id">{{item.saleAttrValueName}}</el-tag>
          </el-col>
        </el-row>
        <el-row style="margin: 10px 0;">
