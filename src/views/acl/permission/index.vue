...
import {MenuParams, Permission, PermissionList, PermissionResponseData} from "@/api/acl/menu/type.ts";
...
<template>
  <el-table :data="permissionArr" style="width: 100%; margin-bottom: 20px" row-key="id" border>
    ...
    <el-table-column label="操作" align="center">
      <template #="{row,$index}">
        <el-button type="primary" size="small" :disabled="row.level==4?true:false" @click="addPermission(row)">{{row.level==3? '添加功能':'添加菜单' }}</el-button>
        <el-button type="primary" size="small" :disabled="row.level==1?true:false" @click="updatePermission(row)">编辑</el-button>
        <el-popconfirm :title="`你确定要删除${row.name}吗?`" width="250px" @confirm="removeMenu(row.id)">
          <template #reference>
  ...
  <el-dialog v-model="dialogVisible" :title="menuData.id?'更新菜单':'添加菜单'" width="500px">
    <el-form>
      ...
      <el-form-item label="权限值" label-width="60px">
        <el-input placeholder="请你输入菜单权限值" v-model="menuData.code"></el-input>
      </el-form-item>
    </el-form>
    <template #footer>
